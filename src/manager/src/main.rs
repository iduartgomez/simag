...
impl<'a> Manager<'a> {
    fn new() -> Manager<'a> {
        Manager { messages: Box::new(ConsoleMsg::new()) }
    }
...
impl<'a> ConsoleMsg<'a> {
    fn new() -> ConsoleMsg<'a> {
        ...
        let msg: &[ANSIString] = &[Red.paint("Unknown command, write "),
        ...
        messages.insert("arg_num",
                        "Wrong number of arguments ({}), for {} command, write {} for help"
                            .to_string());

        ConsoleMsg { messages: messages }
    }
